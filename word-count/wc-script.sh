...
head | out/mobydick_count_local.txt
...
# run it on hadoop
hadoop jar $HADOOP_HOME/hadoop-streaming.jar $PARAMS -mapper mapper.py -reducer reducer.py -input count_example/mobydick.txt -output count_example/mobydick_out -file mapper.py -file reducer.py 

# view the output 
mkdir out
hadoop fs -ls count_example/mobydick_out/
hadoop fs -cat count_example/mobydick_out/part-* | sort -nrk 2 > out/mobydick_count.txt
